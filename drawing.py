...
from .state import _session, HANDLE_SELECT_RADIUS, SAFE_LIMIT, MIN_HANDLE_SCALE, MAX_HANDLE_SCALE, get_addon_prefs
...
def render_keyframe_control_handles(context, point_3d, frame_kf_map, shader, styles, frame_num, bone=None, parent_matrix=None, collector=None, obj_name=None):
    """渲染关键帧控制手柄"""
    global _session
    wm = context.window_manager
    ...
    is_selected = (_session.active_handle_index == len(_session.handle_control_points) - 1 and 
                  _session.handle_edit_in_progress)
    ...
    if num_added > 0:
        for i in range(num_added):
            ...
            is_selected = (idx == _session.active_handle_index and _session.handle_edit_in_progress)
            if is_selected:
...
def render_keyframe_marker(context, point_3d, frame_num,
    ...
    if is_selected_keyframe:
        if is_keyframe_point and frame_kf_map:
            render_keyframe_control_handles(context, point_3d, frame_kf_map, shader, styles, frame_num, bone=bone, parent_matrix=parent_matrix, collector=collector, obj_name=obj_name)
